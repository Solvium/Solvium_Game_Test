...
export function formatTime(time: number) {
  ...
  let result = `${formatNumber(
    Math.floor((absTime / 6e4) % 60),
    2
  )}:${formatNumber(Math.floor((absTime / 1e3) % 60), 2)}`;
  if (time < 36e5) return (time < 0 ? "-" : "") + result;
  result = `${formatNumber(Math.floor((absTime / 36e5) % 24), 2)}:${result}`;
  if (time < 864e5) return (time < 0 ? "-" : "") + result;
  return `${formatNumber(Math.floor(time / 864e5), 2)}d ${result}`;
}
...
export function formatDateTime(date: Date = new Date()) {
  ...
  return `${formatNumber(dy, 2)}/${formatNumber(mo + 1, 2)}/${formatNumber(
    yr,
    4
  )} ${formatNumber(hr % 12 || 12, 2)}:${formatNumber(mi, 2)}:${formatNumber(
    se,
    2
  )} ${hr >= 12 ? "PM" : "AM"}`;
}
...
export function toPromise<S, T>(
  thisArg: S,
  src: (this: S, callback: (result: T) => void) => void
): Promise<T>;
...
export function findStyle(
  selector: string,
  root: ParentNode & Node = window.document
) {
  ...
  for (let i = styleElements.length - 1; i >= 0; i--) {
    ...
    for (let j = sheet.rules.length - 1; j >= 0; j--) {
      ...
      for (const targetSelector of (rule as CSSStyleRule).selectorText.split(
        ...
        if (targetSelector.trim() === selector) return rule as CSSStyleRule;
    }
  }
  if (!lastSheet) {
    ...
    lastSheet = (
      document.documentElement instanceof SVGElement
        ? (document.appendChild(
            document.createElementNS(NS_SVG, "style")
          ) as SVGStyleElement & LinkStyle)
        : document.head.appendChild(document.createElement("style"))
    ).sheet!;
  }
  const index = lastSheet.insertRule(
    `${selector} {}`,
    lastSheet.rules.length - 1
  );
